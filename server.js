...
app.post("/api/leaderboard", (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      error:
        "name (string), score (number), and playerId (string) required"
    });
  }
  ...
  if (nameTakenByOther) {
    return res.status(409).json({
      error:
        "That name is already taken by another player."
    });
  }

  const existing =
    leaderboard.find(
      e => e.ownerId === cleanOwnerId
    );
...
app.post(
  ...
  (req, res) => {
    ...
    // Tell connected players that a reset happened.
    broadcastEvent({
    ...
    res.json({
      ok: true,
      message:
        "All player leaderboard progress has been reset.",
      owner: OWNER_NAME,
      title: OWNER_TITLE
    });
  }
...
app.post(
  ...
  (req, res) => {
    ...
    ----------------------------------------- */
      ...
      if (
        !isFinite(clicksPerSecond) ||
        clicksPerSecond <= 0
      ) {
        return res.status(400).json({
          error:
            "clicksPerSecond must be positive"
        });
      }
    ...
    cleanPayload.adminName =
      OWNER_NAME;

    cleanPayload.adminTitle =
      OWNER_TITLE;

    cleanPayload.adminBadge =
      OWNER_BADGE;
...
server.listen(
  ...
  () => {
    ...
    console.log(
      "🔥 Ember Foundry server listening on port " +
      PORT
    );
